...
select sub_ship.shipping_id, sub_ship.vendor_id, sub_ship.payment_amount, sub_ship.shipping_plan_datetime, st.id as transfer_id, sub_ship.agreement_id, sc.country_rate_id from (
    select shipping_id, vendor_id, payment_amount, shipping_plan_datetime,
           cast((regexp_split_to_array(shipping_transfer_description,E'\:'))[1] as text) as transfer_id,
           (regexp_split_to_array(vendor_agreement_description,E'\:'))[1]::bigint as agreement_id,
           shipping_country
       from shipping) as sub_ship
    inner join (select id, transfer_type from shipping_transfer) as st on sub_ship.transfer_id=st.transfer_type
...
with final_state as (
    select shipping_id, max(case when state = 'recieved' then state_datetime else null end) as last_time from shipping group by 1
),
first_state as (
    select shipping_id, min(case when state = 'booked' then state_datetime else null end) as first_time from shipping group by 1
)
select shipping_id, status, state, firs.first_time, fins.last_time from shipping as s
    join final_state fins using(shipping_id)
    join first_state firs using(shipping_id)
where cast(s.state_datetime as text)=cast(fins.last_time as text);
...
from shipping_status ss
    join shipping_info si using (shipping_id)
